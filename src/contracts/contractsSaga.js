...
function initializeContract(contractArtifact) {
  ...
  .catch((error) => {
    console.error('Error in ' + contract.contractArtifact.contract_name + ': ' + cFunction)
    console.error(error)
...
function* callInitializeContract(action) {
  ...
  if (!contractInfo) {
    yield call(action.reject, {source: 'contracts', message: 'Failed to initialize contract.'})
  }
...
function getContractVar({contract, i, name}) {
  ...
  console.log(contract.contractArtifact.contract_name + ': ' + contract.abi[i].name)

  return contract.methods[name]().call({from: store.getState().accounts[0]}, (error, result) => {
    if (error) {
      console.error('Error in ' + contract.contractArtifact.contract_name + ': ' + contract.abi[i].name)
      console.error(error)
