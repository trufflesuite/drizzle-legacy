...
describe("Accounts Saga", function () {
  ...
  it('gets accounts', function() {
    ...
    let next = gen.next();
    assert.deepEqual(next.value, call(web3.eth.getAccounts), 'must call getAccounts');
    
    next = gen.next(accounts);
    assert.deepEqual(next.value, put({ type: 'ACCOUNTS_FETCHED', accounts }), 'must dispatch accounts');
